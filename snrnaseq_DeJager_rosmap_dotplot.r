...
#celltype AD vs Control by APOE4status z_score_transform
for(celltype in celltypes){
#seu <- LoadH5Seurat(paste0(work_dir,celltype,'/',celltype,'.h5Seurat'))
#seuObj = load(paste0(work_dir,celltype,'_seu.rda'))
seuObj = load('/home/shann/Documents/synapse/ROSMAP/snrnaseq/DeJager/seurat_files/endo_seu.rda')
...
dot_df$cell_exp_ct = cel_exp_ct_long$cell_exp_ct
dot_df = dot_df[!dot_df$celltype %in% grep("_NA$", dot_df$celltype, invert=FALSE, value = TRUE),]
dot_df_lst = split(dot_df,dot_df$substructure)
dot_df_lst2 = lapply(dot_df_lst,function(x){x$cell_exp_zscore = z_score_transform(x$cell_exp);return(x)})
dot_df2 = do.call(rbind,dot_df_lst2)
midpoint = median(dot_df2$cell_exp_zscore)
#celltype order
#celltypes = unique(meta$state)
#celltypes = celltypes[order(celltypes)]
#celltypes = rep(celltypes,each=4)
groups = c('Control_APOE4noncarrier','Control_APOE4carrier','AD_APOE4noncarrier','AD_APOE4carrier')
groups = groups[groups %in% dot_df2$celltype]
dot_df2$celltype = factor(dot_df2$celltype,levels = groups)
#graph
p = ggplot(dot_df2, aes(x=celltype, y = substructure, color = cell_exp_zscore, size = cell_exp_pct)) +
  geom_point() + geom_text(label = dot_df2$cell_exp_ct, nudge_x = 0, nudge_y = 0.25, check_overlap = T, color = 'black', size = 2) +
  scale_colour_gradient2(name = 'cell exp', low = 'blue', mid = 'white smoke', high = 'red', midpoint = midpoint) +
  cowplot::theme_cowplot() +
  theme(axis.line  = element_blank()) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  ylab('') +
  theme(axis.ticks = element_blank())
pdf(file = paste0(out_dir, 'DeJager_rosmap_snRNAseq_',celltype,'_diagnosis_by_APOE4status_dotplot_zscore_transformed.pdf'), width = 1*length(groups), height = 5)
print(p)
dev.off()
}


#celltype AD vs Control by APOE4status CP IP seperated with z-score transformation
for(celltype in celltypes){
#seu <- LoadH5Seurat(paste0(work_dir,celltype,'/',celltype,'.h5Seurat'))
#seuObj = load(paste0(work_dir,celltype,'_seu.rda'))
seuObj = load('/home/shann/Documents/synapse/ROSMAP/snrnaseq/DeJager/seurat_files/endo_seu.rda')
